(define-library (match-test)
  ...
  (import (except (scheme base) equal?)
	  (scheme write)
          (cyclone0.19 srfi-204))
  (cond-expand
   (chibi
    (import (only (chibi process) process->string)
	    (srfi 115)
	    (only (chibi test)
		test-begin
		test
   ...
   (cyclone
     ...
     (begin
       (define (version)
	 "get version string"
	 (match-let loop (((this . others) (map symbol->string (features))))
		    (match (string->list this)
			   ((#\v #\e #\r #\s #\i #\o #\n . num)
			    (string-append "cyclone" (list->string num)))
			   (_ (if (null? others) "cyclone-???" (loop others))))))
       (display (version))
       (newline)
       (define-record-type Point
       (make-point x y)
